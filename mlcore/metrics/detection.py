"""
Copied from
https://github.com/Peterande/D-FINE/blob/master/src/solver/validator.py
"""
...
@COMPONENTS.register(
    ...
    description="Computes general metrics such as <f1, recall, precison, iou>",
)
class ObjectDetMetricsTracker(BaseMetricTracker):
    def __init__(
        self,
        conf_thresh=0.5,
        iou_thresh=0.5,
    ...
    def reset(self):
        ...
        self.gt: list[dict[str, torch.Tensor]] = []
    ...
    def update(self, predict: list[dict], gt: list[dict]):
        for target in gt:
            ...
            target["boxes"] = scale_boxes(
                boxes, target["orig_size"], boxes.canvas_size[::-1]
            )

        self.gt.extend(gt)
        self.preds.extend(predict)

    def compute(self):
        filtered_preds = filter_preds(copy.deepcopy(self.preds), self.conf_thresh)
        self.metrics = self._compute_main_metrics(filtered_preds)

    def average(self) -> dict[str:float]:
        return self.metrics["average"]

    def per_cls_metrics(self) -> dict[str, float]:
        return self.metrics["per_cls"]

    def _compute_main_metrics(self, preds) -> Metrics:
        (
        ...
        ious = []
        tps = fps = fns = 0
        extended_metrics = dict(precision={}, recall={}, iou={})

        for cls_id, value in self.metrics_per_class.items():
            cls_tps, cls_fps, cls_fns = value["TPs"], value["FPs"], value["FNs"]

            cls_precision = (
                cls_tps / (cls_tps + cls_fps) if cls_tps + cls_fps > 0 else 0.0
            )
            cls_recall = cls_tps / (cls_tps + cls_fns) if cls_tps + cls_fns > 0 else 0.0

            extended_metrics["precision"][cls_id] = cls_precision
            extended_metrics["recall"][cls_id] = cls_recall
            extended_metrics["iou"][cls_id] = np.mean(value["IoUs"])
