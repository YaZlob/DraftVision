...
from mlcore.models.layers.utils import (
    bbox2distance,
    box_cxcywh_to_xyxy,
    generalized_box_iou,
    box_iou,
...
@COMPONENTS.register(
    ...
    description="Hungarian Matcher for Detr like object detectors",
)
class HungarianMatcher(nn.Module):
    ...
    def __repr__(self):
        return (
            ...
            f"Cost class: {self.cost_class}\n"
            f"Cost bbox: {self.cost_bbox}\n"
            f"Cost giou: {self.cost_giou}\n"
            f"Focal loss: {self.use_focal_loss}, alpha= {self.alpha}, gamma= {self.gamma}"
        )
...
class DFINECriterion(nn.Module):
    ...
    def __init__(
        ...
        if not isinstance(matcher, HungarianMatcher):
            raise ValueError(
                f"{self.__class__.__name__} works only with HungarianMatcher, got {type(matcher)}"
            )
    ...
    def __repr__(self):
        return (
            ...
            f"Weight dict: {self.weight_dict}\n"
            f"Losses: {self.losses}\n"
            f"boxes weight format: {self.boxes_weight_format}\n"
            f"share matched indices: {self.share_matched_indices}\n"
            f"alpha={self.alpha}, gamma={self.gamma}, reg_max={self.reg_max}\n"
